...
- [x] 1.2 建立自动发布闸门 — _需求: FR-9_
  - 证据块：发布闸门串联静态检查、全量测试、生产构建、Legacy/Modern 生产冒烟、Agent Evals 和 npm 包清单；任一错误级检查失败即阻断。
  - 实现：新增 `src/release/release-readiness.ts`、`release:static` 与 `release:verify`；校验 Node 20、SDK 2.0.0 精确版本、旧 SDK 移除、33 工具、Plan 工具、包文件和迁移材料。
  - 验证：`npm run release:verify` exit 0；80 个测试文件/379 项通过，构建、双协议冒烟、24 项 Evals、`npm pack --dry-run` 全部通过。当前 3.7.0 → 4.x 版本提升被保留为 warning，不伪装为已发布。
- [x] 2.1 完成迁移与客户端矩阵 — _需求: FR-9_
  - 证据块：v3 → v4 迁移说明覆盖 Node 20、SDK v2、协议模式、Tasks 与 input_required 降级、Plan 状态、Converge 后置记忆和回退；真实客户端未验证项全部保持 pending。
